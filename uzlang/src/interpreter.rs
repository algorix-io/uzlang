        ...
        match expr {
            ...
            Expr::Call(name, args) => {
                ...
                match name.as_str() {
                    ...
                    "qosh" => {
                        // Bolt: Use into_iter() and Rc::make_mut for O(1) amortized in-place update if unshared
                        let mut args_iter = arg_values.into_iter();
                        if let Some(arr_val) = args_iter.next() {
                            if let Value::Array(mut rc_arr) = arr_val {
                                if let Some(new_val) = args_iter.next() {
                                    let elements = Rc::make_mut(&mut rc_arr);
                ...
                if let Some((params, body)) = self.functions.get(name) {
                    ...
                    // Create new scope
                    // Bolt: Use into_iter() to transfer arguments into the scope without cloning
                    let mut scope = HashMap::with_capacity(params.len());
                    let mut args_iter = arg_values.into_iter();
                    for param in params.iter() {
                        let val = args_iter.next().unwrap_or(Value::Number(0));
