                    ...
                    if let Some(val) = scope.get_mut(name.as_str()) {
                        ...
                        if let Value::Array(rc_arr) = val {
                            if let Value::Number(idx) = index_val {
                                // Bolt: AssignIndex already uses Rc::make_mut but is only possible because we have &mut val.
            ...
            Expr::Call(name, args) => {
                ...
                match name.as_str() {
                    ...
                    "qosh" => {
                        ...
                        // Bolt: Optimize qosh to use into_iter() and Rc::make_mut for O(1) amortized in-place updates.
                        if let (Some(Value::Array(rc_arr)), Some(val)) =
                            (arg_iter.next(), arg_iter.next())
                        {
                            let mut arr = rc_arr;
                            {
                                let elements = Rc::make_mut(&mut arr);
                                elements.push(val);
                            }
                            return Value::Array(arr);
                        } else {
                            eprintln!("Xatolik: 'qosh' funksiyasi argumentlari noto'g'ri");
                        }
                        return Value::Number(0);
                    }
                    "internet_ol" => {
                    ...
                    "internet_yoz" => {
                        if let (Some(url_val), Some(json_val)) = (arg_iter.next(), arg_iter.next()) {
