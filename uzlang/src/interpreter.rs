    ...
    fn evaluate(&mut self, expr: &Expr) -> Value {
        match expr {
            ...
            Expr::Call(name, args) => {
                ...
                // Bolt: Use into_iter() to avoid cloning values when passing to functions
                ...
                match name.as_str() {
                    ...
                    "qosh" => {
                        ...
                        // Bolt: Use Rc::make_mut for O(1) in-place update if the array is unshared
                        if let Some(Value::Array(mut rc_arr)) = args_iter.next() {
                            if let Some(new_val) = args_iter.next() {
                                Rc::make_mut(&mut rc_arr).push(new_val);
                    ...
                    "internet_yoz" => {
                        if let (Some(url_val), Some(json_val)) = (args_iter.next(), args_iter.next())
                        {
